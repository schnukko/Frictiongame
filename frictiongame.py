        ...
        while running:
            for event in pygame.event.get():
                ...
                # ------- pressed and released key ------
                    ...
                    if event.key == pygame.K_1:
                        PygView.friction = 1.00001
                    if event.key == pygame.K_2:
                        PygView.friction = 1
                    if event.key == pygame.K_3:
                        PygView.friction = 0.999995
                    if event.key == pygame.K_4:
                        PygView.friction = 0.99995
                    if event.key == pygame.K_5:
                        PygView.friction = 0.9995
                    if event.key == pygame.K_6:
                        PygView.friction = 0.995
                    if event.key == pygame.K_7:
                        PygView.friction = 0.95
                    if event.key == pygame.K_8:
                        PygView.friction = 0.925
            ...
            if self.gametime <= 0:
                if self.score2 == self.score1:
                    ...
                    text = 'red player wins ^0^'
                elif self.score1 < self.score2:
                    text = 'blue wins! ^0^'
                write(self.screen, text, x=PygView.width//2,
                    y=PygView.height//2, fontsize=100,center=True)
                pygame.display.flip()
                time.sleep(2)
                break
            ...
            write(self.screen, "FPS: {:6.3}  , GAMETIME{:6.4}, FRICTION =:{}   r = reset, 1-10 = change friction ".format(
                           self.clock.get_fps(), self.gametime, self.friction))
...
if __name__ == '__main__':
    try:
        g = int(input('How many seconds do you want to play?'))
    except:
        g = 120
    PygView(1430,800, friction=0.99, gametime = g).run() # try PygView(800,600).run()
